...
def VoronoiDiagramInterpolation(interpolationcellid, id0, id1, voronoiDataset0,
    ...
    if distance(endPoint, cellLine.GetPoint(endId)) > 0.1:
...
def remove_points(newVoronoiPoints, newVoronoiPointsMISR, bif,
    ...
    lines = []
    for i in range(2):
        lines.append(ExtractSingleLine(interpolated_cl, i))
    ...
    segment = ExtractSingleLine(lower_line, 0, startID=startID, endID=endID)
...
def tmp_InsertNewVoronoiPoints(oldDataset):
    numberOfDatasetPoints = oldDataset.GetNumberOfPoints()
    numberOfNewVoronoiPoints = numberOfDatasetPoints
...
def create_new_surface(completeVoronoiDiagram):
    ...
    #marchingCube->SetInputConnection(modeller.GetOutput()->GetProducerPort());
...
def interpolate_voronoi_diagram(interpolatedCenterlines, patchCenterlines,
    ...
    for j in range(1, 2): #numberOfInterpolatedCenterlinesCells + 1):
        test=0
        #print(j)
        ...
        # Interpolate Voronoi
        # Check where it contributes, and why other do not
        #WritePolyData(startInterpolationDataset, "%s_startInterpolationDataSet.vtp" % (str(j)))
        #WritePolyData(startHalfInterpolationDataset, "%s_startHalfInterpolationDataSet.vtp" % (str(j)))
        ...
        #WritePolyData(endInterpolationDataset, "%s_endInterpolationDataSet.vtp" % (str(j)))
        #WritePolyData(endHalfInterpolationDataset, "%s_endHalfInterpolationDataSet.vtp" % (str(j)))
