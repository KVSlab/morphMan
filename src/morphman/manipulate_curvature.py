...
def manipulate_curvature(
    ...
    smoothed_centerline_splined = vmtk_compute_geometric_features(
        centerline_splined, True, True, factor=smooth_factor_line, iterations=iterations
    )
    ...
    diverging_points = [
        centerline_diverging[i].GetPoint(diverging_ids[i])
        for i in range(len(diverging_ids))
    ]
    moved_voronoi_region, div_offset = make_voronoi_smooth(
        voronoi_regions[0],
        centerline_splined,
    ...
    new_centerlines = move_all_centerlines(
        centerlines, smoothed_centerline_splined, smooth_line, div_offset
    )
...
def make_voronoi_smooth(
    voronoi, centerline, smoothed_centerline, smooth_line, div_voronoi, div_points
):
    """
...
def move_all_centerlines(
    unsmoothed_centerline, smoothed_centerline, smooth_line, div_offset
):
    """
...
def read_command_line_curvature(input_path=None, output_path=None):
    ...
    description = (
        "Manipulates a selected part of a tubular geometry"
    ...
    return dict(
        ...
        smooth_factor_line=args.smooth_factor_line,
