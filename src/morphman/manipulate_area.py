...
def manipulate_area(
    ...
    (
        ...
        centerline_diverging,
        region_points,
        diverging_ids,
    ) = get_line_to_change(
        capped_surface, centerlines, region_of_interest, method, region_points, size
    )
    ...
    if centerline_diverging is not None:
        write_polydata(
            vtk_merge_polydata(centerline_diverging), centerline_diverging_path
        )
    ...
    if centerline_diverging is not None:
        for i, div_cl in enumerate(centerline_diverging):
            centerline_regions += [
                extract_single_line(div_cl, 0, start_id=diverging_ids[i])
            ]
    ...
    if centerline_diverging is not None or smooth:
        ...
        surface_area = create_new_surface(
            voronoi_regions[0],
    ...
    centerline_area, centerline_area_sections = vmtk_compute_centerline_sections(
        surface_area, centerline_splined
    )
    ...
    new_voronoi, new_centerlines = change_area(
        voronoi_regions[0],
        factor,
        centerline_area,
        centerline_diverging,
    ...
    new_surface = prepare_output_surface(
        new_surface, surface, centerlines, output_filepath, test_merge=True
    )
...
def get_factor(
    ...
    if region_of_interest in ["manual", "commandline"] and method in [
        "area",
        "variation",
    ]:
    ...
    # Transition
    trans = np.asarray(
        np.linspace(1, 0, k).tolist()
        + np.zeros(linear).tolist()
...
def change_area(
    ...
    for i in range(n):
        ...
        if AB_length > tol * 10:
            h = (
                np.linalg.norm(np.cross(AP, AB)) / AB_length
        ...
        if angle_asymmetric is not None:
            v = get_asymmetric_displacement(
                A,
                angle_asymmetric,
                factor_,
                frenet_normals_array,
                frenet_tangents_array,
                locator,
                point,
            )
        else:
    ...
    if diverging_centerline is not None:
        ...
        new_centerlines.GetPointData().AddArray(
            centerlines.GetPointData().GetArray(radiusArrayName)
        )
        ...
        for j, div_voronoi in enumerate(diverging_voronoi):
            ...
            start_region_of_interest_id = loc_div_cl.FindClosestPoint(
                line_to_change.GetPoint(0)
            )
            tmp_diverging_centerline = extract_single_line(
            ...
            for i in range(tmp_diverging_centerline.GetNumberOfPoints()):
                ...
                dist = get_distance(
                    surface_area.GetPoint(loc_surf.FindClosestPoint(point)), point
                )
                if dist < min_dist:
                    min_point = point
                    min_dist = dist
            ...
            if AC_length != 0:
                h = (
                    np.linalg.norm(np.cross(BA, BC)) / AC_length
                )  # shortest distance between point and line
            ...
            delta_p = D - B
            factor_ = factor[tmp_id1] * (1 - np.linalg.norm(D) / AC_length) + factor[
                tmp_id2
            ] * (np.linalg.norm(D) / AC_length)
            v = delta_p * (1 - factor_)
            ...
            point_radius_array = (
                div_voronoi.GetPointData().GetArray(radiusArrayName).GetTuple1
            )
...
def get_asymmetric_displacement(
    A,
    angle_asymmetric,
    factor_,
    frenet_normals_array,
...
def update_factor(A, AB_length, B, P_mid, factor, tmp_id1, tmp_id2):
    ...
    factor_ = (
        factor[tmp_id1] * AP_mid_length + factor[tmp_id2] * BP_mid_length
    ) / AB_length
...
def read_command_line_area(input_path=None, output_path=None):
    ...
    description = (
        "Manipulates the area of a tubular geometry. The script changes the area"
    ...
    parser.add_argument(
        ...
        + "experimental for other methods.",
        metavar="angle-asymmetric",
    )
    ...
    if (
        args.method in ["stenosis", "bulge", "linear"]
        and args.region_of_interest == "first_line"
    ):
        raise ValueError(
            "Can not set region of interest to 'first_line' for 'stenosis', 'bulge', or 'linear'"
        )

    if args.method == "variation" and args.ratio is not None and args.beta != 0.5:
        print(
            "WARNING: The beta value you provided will be ignored, using ratio instead."
        )

    if args.region_points is not None:
        if len(args.region_points) % 3 != 0 or len(args.region_points) > 6:
            raise ValueError(
                "ERROR: Please provide region point(s) as a multiple of 3, and maximum two points."
            )
    ...
    return dict(
        ...
        angle_asymmetric=angle_radians,
