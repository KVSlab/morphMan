...
from vmtk.vtkvmtkCommonPython import vtkvmtkMath
from vmtk.vtkvmtkComputationalGeometryPython import vtkvmtkCenterlineAttributesFilter

from morphman.common.tools_common import get_distance
...
def create_parent_artery_patches(
    parentCenterlines, clipPoints, siphon=False, bif=False
):
    ...
    if siphon:
        clipIds, numberOfPatchedCenterlinesPoints = extract_patches_ids_siphon(
            parentCenterlines, clipPoints
        )
    else:
        clipIds, numberOfPatchedCenterlinesPoints = extract_patches_ids(
            parentCenterlines, clipPoints
        )
...
def interpolate_patch_centerlines(
    patchCenterlines,
    parentCenterlines,
    additionalPoint,
    lower,
    ...
    for i in range(parentCenterlines.GetNumberOfCells()):
        ...
        if version:
            ...
            splinePoints = interpolate_two_cells(
                startingCell,
                endingCell,
    ...
    attributeFilter = vtkvmtkCenterlineAttributesFilter()
...
def interpolate_spline(startCell, endCell, additionalPoint):
    ...
    for point in range(len(tmp)):
        points.SetPoint(point, tmp[point])

    return points


def interpolate_two_cells(
    startCell,
    endCell,
    ...
    for i in range(numberOfSplinePoints):
        points.SetPoint(
            i,
            xspline.Evaluate(float(i)),
            yspline.Evaluate(float(i)),
            zspline.Evaluate(float(i)),
        )
    return points


def extract_cylindric_interpolation_voronoi_diagram(
    cellId, pointId, cylinderRadius, voronoi, centerlines, interpolationHalfSize=3
):
    ...
    for i in range(voronoi.GetNumberOfPoints()):
        ...
        isInside = is_point_inside_interpolation_cylinder(
            point, cylinderTop, cylinderCenter, cylinderBottom, cylinderRadius
        )
...
def is_point_inside_interpolation_cylinder(x, t, c, b, r):
    ...
    alpha = np.arccos(vtk.vtkMath.Dot(xc, tb))

    parallelxc = xcnorm * np.cos(alpha)
    perpendicularxc = xcnorm * np.sin(alpha)

    thetamin = np.arctan(r / halfheigth)
    thetamax = thetamin + (np.pi - 2 * thetamin)
...
def voronoi_diagram_interpolation(
    interpolationcellid,
    id0,
    id1,
    voronoiDataset0,
    ...
    numberOfGapPoints = int(np.abs(gapEndId - gapStartId)) + 1
    ...
    for i in range(numberOfInterpolationPoints):
        ...
        voronoiPointRadius = (
            voronoiDataset0.GetPointData().GetArray(radiusArrayName).GetTuple1(i)
        )
        ...
        rotationAngle = compute_voronoi_vector_to_centerline_angle(
            closestPointId, voronoiVector, cellLine
        )
        ...
        for j in range(closestPointId, arrivalId, range_step):
            ...
            localnormal = (
                cellLine.GetPointData()
                .GetArray(parallelTransportNormalsArrayName)
                .GetTuple3(j)
            )
        ...
        arrivalVoronoiPointRadius = (
            voronoiDataset1.GetPointData()
            .GetArray(radiusArrayName)
            .GetTuple1(arrivalVoronoiPointId)
        )

        arrivalCenterlinePointLocator = get_vtk_point_locator(cellLine)

        arrivalCenterlineClosestPointId = (
            arrivalCenterlinePointLocator.FindClosestPoint(arrivalVoronoiPoint)
        )
        arrivalCenterlineClosestPoint = cellLine.GetPoint(
            arrivalCenterlineClosestPointId
        )

        arrivalVoronoiVector = [0.0, 0.0, 0.0]
        arrivalVoronoiVector[0] = (
            arrivalVoronoiPoint[0] - arrivalCenterlineClosestPoint[0]
        )
        arrivalVoronoiVector[1] = (
            arrivalVoronoiPoint[1] - arrivalCenterlineClosestPoint[1]
        )
        arrivalVoronoiVector[2] = (
            arrivalVoronoiPoint[2] - arrivalCenterlineClosestPoint[2]
        )
        arrivalVoronoiVectorNorm = vtk.vtkMath.Norm(arrivalVoronoiVector)
        radiusArray = compute_spline(
            voronoiPointRadius, arrivalVoronoiPointRadius, numberOfPTPoints
        )
        vectorNormArray = compute_spline(
            voronoiVectorNorm, arrivalVoronoiVectorNorm, numberOfPTPoints
        )
...
def compute_angle_between_vectors(normal, tangent, vector):
    ...
    theta = vtkvmtkMath.AngleBetweenNormals(normal, ovector)
...
def interpolate_voronoi_diagram(
    ...
    for j in range(1, 3):
        ...
        startCellPointRadius = (
            patchCenterlines.GetPointData()
            .GetArray(radiusArrayName)
            .GetTuple1(startCellPointId)
        )
        startCellPointHalfRadius = startCellPointRadius / cylinder_factor

        startInterpolationDataset = extract_cylindric_interpolation_voronoi_diagram(
            startId,
            startCellPointId,
            startCellPointRadius,
            clippedVoronoi,
        ...
        startHalfInterpolationDataset = extract_cylindric_interpolation_voronoi_diagram(
            startId,
            startCellPointId,
        ...
        endInterpolationDataset = extract_cylindric_interpolation_voronoi_diagram(
            endId, endCellPointId, endCellPointRadius, clippedVoronoi, patchCenterlines
        )
        endHalfInterpolationDataset = extract_cylindric_interpolation_voronoi_diagram(
            endId,
            endCellPointId,
        ...
        newVoronoiPoints, newVoronoiPointsMISR = voronoi_diagram_interpolation(
            interpolationCellId,
            startId,
            endId,
        ...
        completeVoronoiDiagram = insert_new_voronoi_points(
            completeVoronoiDiagram, newVoronoiPoints, newVoronoiPointsMISR
        )
    ...
    if bif is not []:
        for i in range(len(bif) - 1):
            ...
            startR = (
                startCell.GetPointData().GetArray(radiusArrayName).GetTuple1(startId)
            )
            ...
            startInterpolationDataset = extract_cylindric_interpolation_voronoi_diagram(
                startId_, startId, startR, clippedVoronoi, startCell
            )
            startHalfInterpolationDataset = (
                extract_cylindric_interpolation_voronoi_diagram(
                    startId_, startId, startRHalf, clippedVoronoi, startCell
                )
            )

            endInterpolationDataset = extract_cylindric_interpolation_voronoi_diagram(
                endId_, endId, endR, clippedVoronoi, endCell
            )
            endHalfInterpolationDataset = (
                extract_cylindric_interpolation_voronoi_diagram(
                    endId_, endId, endRHalf, clippedVoronoi, endCell
                )
            )
            newVoronoiPoints, newVoronoiPointsMISR = voronoi_diagram_interpolation(
                interpolationCellId,
