...
def vmtk_compute_centerline_sections(surface, centerlines):
    ...
    """
    centerline_sections = vtkvmtkPolyDataCenterlineSections()
...
def vmtk_compute_geometric_features(
    centerlines, smooth, output_smoothed=False, factor=1.0, iterations=100
):
...
def vmtk_cap_polydata(
    surface, boundary_ids=None, displacement=0.0, in_plane_displacement=0.0
):
    ...
    """
    surface_capper = vtkvmtkCapPolyData()
...
def vmtk_polyball_modeller(voronoi_diagram, poly_ball_size):
    ...
    """
    modeller = vtkvmtkPolyBallModeller()
...
def vmtk_surface_connectivity(
    surface, method="largest", clean_output=True, closest_point=None
):
    """
...
def vmtk_branch_clipper(
    centerlines,
    surface,
    clip_value=0.0,
    inside_out=False,
    use_radius_information=True,
    interactive=False,
):
    """
...
def vmtk_endpoint_extractor(
    centerlines, number_of_end_point_spheres, number_of_gap_spheres=1
):
    """
...
def vmtk_compute_surface_normals(
    surface,
...
def vmtk_surface_curvature(
    surface,
    curvature_type="mean",
    absolute=False,
    median_filtering=False,
    curvature_on_boundaries=False,
    bounded_reciporcal=False,
    epsilon=1.0,
    offset=0.0,
):
