...
from morphman.common.vmtk_wrapper import (
    vmtk_branch_clipper,
    vmtk_cap_polydata,
    vmtk_compute_centerlines,
    vmtk_compute_voronoi_diagram,
    vmtk_endpoint_extractor,
    vmtk_resample_centerline,
    vmtk_smooth_centerline,
    vmtk_smooth_surface,
    vmtk_surface_connectivity,
)
...
from morphman.common.vtk_wrapper import (
    ...
    vtk_compute_normal_gradients,
    vtk_compute_polydata_normals,
...
def compute_centers(polydata, case_path=None, select_inlet=False):
    ...
    for i in range(int(region_array.max()) + 1):
        ...
        boundary_points = vtk_compute_threshold(
            outputs,
            "RegionId",
            lower=i - 0.1,
            upper=i + 0.1,
...
def provide_inlet(surface):
    ...
    """
    print(
        "-- Please select the boundary representing the inlet in the interactive window."
    )
...
def provide_relevant_outlets(surface, dir_path=None):
    ...
    get_point = surface.GetPoints().GetPoint
    points = [
        list(get_point(point_seed_ids.GetId(i)))
        for i in range(point_seed_ids.GetNumberOfIds())
    ]
    info = {}
...
def get_uncapped_surface(
    surface, gradients_limit=0.15, area_limit=0.3, circleness_limit=3
):
    """
    ...
    # Extract capps
    end_capps = vtk_compute_threshold(
        gradients,
        "Gradients_mag",
        lower=0.5,
        upper=1.5,
    ...
    mark_outlets = create_vtk_array(
        np.zeros(surface.GetNumberOfCells()), "outlets", k=1
    )
    ...
    uncapped_surface = vtk_compute_threshold(
        surface, "outlets", lower=0, upper=0.5, threshold_type="between", source=1
    )
    ...
    while remove:
        ...
        mark_outlets = create_vtk_array(
            np.zeros(uncapped_surface.GetNumberOfCells()), "outlets", k=1
        )
        remove = False
...
def check_if_surface_is_merged(surface, centerlines, output_filepath):
    ...
    for i in range(centerlines.GetNumberOfLines()):
        ...
        outlets += lines_to_compare[-1].GetPoint(
            lines_to_compare[-1].GetNumberOfPoints() - 1
        )
    lines_to_check, _, _ = compute_centerlines(
        inlet, outlets, None, surface, resampling=0.1, recompute=True
    )

    for i in range(centerlines.GetNumberOfLines()):
        line_to_compare = vmtk_resample_centerline(lines_to_compare[i], length=0.1)
        line_to_check = vmtk_resample_centerline(
            extract_single_line(lines_to_check, i), length=0.1
        )
        # Compare distance between points along both centerlines
        n = min(
            [line_to_check.GetNumberOfPoints(), line_to_compare.GetNumberOfPoints()]
        )
...
def prepare_output_surface(
    ...
    if changed and old_centerline is None:
        print(
            "WARNING: The changed flag is true, but the old centerline is not provided,"
    ...
    for i in range(region_id.max() + 1):
        ...
        if np.sqrt(np.sum((np.array(inlet_point) - center) ** 2)) < 0.5:
            ...
            line_id = np.argmin(
                np.sqrt(np.sum((np.array(outlets) - center) ** 2, axis=1))
            )
            line = lines[line_id]

        # Set correct direction of normal
        if inlet:
            in_dir = np.array(line.GetPoint(5)) - np.array(line.GetPoint(0))
        else:
            in_dir = np.array(line.GetPoint(line.GetNumberOfPoints() - 5)) - np.array(
                line.GetPoint(line.GetNumberOfPoints() - 1)
            )
        ...
        if changed and old_centerline is not None:
            ...
            if inlet:
                ...
                new_outlet = np.array(
                    new_line.GetPoint(new_line.GetNumberOfPoints() - 1)
                )
                in_dir_new = (
                    np.array(new_line.GetPoint(new_line.GetNumberOfPoints() - 5))
                    - new_outlet
                )
...
def prepare_voronoi_diagram(
    ...
    if voronoi is None:
        voronoi = vmtk_compute_voronoi_diagram(
            capped_surface, base_path + "_voronoi.vtp"
        )
    ...
    if not path.exists(voronoi_smoothed_path) and smooth:
        voronoi = smooth_voronoi_diagram(
            voronoi, centerlines, smooth_factor, no_smooth_cl
        )
...
def compute_centerlines(
    ...
    centerlines, centerlines_output = vmtk_compute_centerlines(
        end_point, inlet, method, outlet, pole_ids, resampling, surface, voronoi
    )

    if smooth:
        centerlines_output = vmtk_smooth_centerline(
            centerlines_output, num_iter, smooth_factor
        )
...
def prepare_surface(base_path, surface_path):
    ...
    if cap_bool:
        ...
        print(
            (
                "WARNING: Tried to automagically uncapp the input surface. Uncapped {}"
                ...
                + " method, or vmtksurfaceendclipper."
            ).format(num_out)
        )
...
def extract_ica_centerline(
    base_path, input_filepath, resampling_step, relevant_outlets=None
):
    """
    ...
    centerline_relevant_outlets_path = (
        base_path + "_centerline_relevant_outlets_landmark.vtp"
    )
    ...
    centerline_relevant_outlets = compute_centerlines(
        inlet,
        outlet1 + outlet2,
        centerline_relevant_outlets_path,
        capped_surface,
...
def get_no_smooth_cl(
    ...
    no_smooth_centerlines, _, _ = compute_centerlines(
        inlet,
        outlets,
        None,
        capped_surface,
    ...
    if region_points is not None:
        ...
        if distance1 < tol * 5 != distance2 < tol * 5:
            no_smooth_centerlines_list.append(
                extract_single_line(line, i, start_id=id1)
            )
    ...
    for i in range(no_smooth_centerlines.GetNumberOfLines()):
        ...
        for j in range(centerlines.GetNumberOfLines()):
            div_ids.append(
                get_diverging_point_id(
                    tmp_line, extract_single_line(centerlines, j), tol
                )
            )
        div_id = max(div_ids)
