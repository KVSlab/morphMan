...
def manipulate_bifurcation(
    ...
    centerline_new_bif_lower_path = (
        base_path + "_centerline_interpolated_bif_lower_ang.vtp"
    )
    ...
    inlet, outlets = get_inlet_and_outlet_centers(
        surface, base_path, select_inlet=select_inlet
    )
    ...
    centerline_par, voronoi, pole_ids = compute_centerlines(
        inlet,
        outlets,
        centerline_par_path,
        capped_surface,
    ...
    centerline_relevant_outlets, _, _ = compute_centerlines(
        inlet,
        outlet1 + outlet2,
    ...
    centerline_bif, _, _ = compute_centerlines(
        outlet1,
        outlet2,
        centerline_bif_path,
        capped_surface,
    ...
    data = get_bifurcating_and_diverging_point_data(
        centerline_relevant_outlets, centerline_bif, tolerance
    )
    ...
    clipped_centerline = get_centerline_between_clipping_points(
        centerline_relevant_outlets, data
    )
    ...
    voronoi_clipped, voronoi_bifurcation = get_split_voronoi_diagram(
        voronoi, [patch_cl, clipped_centerline]
    )
    ...
    interpolated_cl = interpolate_patch_centerlines(
        rotated_cl, centerline_par, div_points[0].GetPoint(0), True, tension, continuity
    )
    write_polydata(interpolated_cl, centerline_new_path.replace(".vtp", "1.vtp"))

    if bif:
        interpolated_bif = interpolate_patch_centerlines(
            rotated_bif_cl, centerline_bif, None, True, tension, continuity
        )
        write_polydata(interpolated_bif, centerline_new_bif_path)

    if lower:
        center = (
            (2 / 9.0) * div_points[1][0]
            + (3.5 / 9.0) * div_points[1][1]
            + (3.5 / 9.0) * div_points[1][2]
        ).tolist()
        print("Center", center)
        div_points[0].SetPoint(0, center[0], center[1], center[2])
        interpolated_bif_lower = interpolate_patch_centerlines(
            rotated_bif_cl,
            centerline_bif,
    ...
    interpolated_cl = merge_cl(interpolated_cl, div_points[1], end_points[1])
    ...
    interpolated_voronoi = interpolate_voronoi_diagram(
        interpolated_cl, rotated_cl, rotated_voronoi, end_points, bif_, cylinder_factor
    )
    # Note: This function is slow, and can be commented, but at the cost of robustness.
    interpolated_voronoi = remove_distant_voronoi_points(
        interpolated_voronoi, interpolated_cl
    )
...
def rotate_voronoi(clipped_voronoi, patch_cl, div_points, m, R):
    ...
    def check_rotate(point):
        ...
        if dist.index(min(dist)) not in not_rotate:
            ...
            return identity, np.array([0, 0, 0])
    ...
    for i in range(number_of_points):
        ...
        point_radius = (
            clipped_voronoi.GetPointData().GetArray(radiusArrayName).GetTuple1(i)
        )
...
def rotation_matrix(data, angle, leave1, leave2):
    ...
    d = (
        np.asarray(data[0]["div_point"])
        + np.asarray(data[1]["div_point"])
        + np.asarray(data["bif"]["div_point"])
    ) / 3.0
    vec = np.eye(3)
    for i in range(2):
    ...
    m1 = np.asarray([[cos_a, -sin_a, 0], [sin_a, cos_a, 0], [0, 0, 1]])
    m2 = np.asarray([[cos_a, sin_a, 0], [-sin_a, cos_a, 0], [0, 0, 1]])
...
def merge_cl(centerline, end_point, div_point):
    ...
    for i in range(2, n_lines):
        ...
        dist1 = math.sqrt(
            np.sum((np.asarray(lines[i].GetPoint(id_1)) - end_point[1]) ** 2)
        )
        dist2 = math.sqrt(
            np.sum((np.asarray(lines[i].GetPoint(id_2)) - end_point[2]) ** 2)
        )
    ...
    for i in range(centerline.GetNumberOfLines()):
        ...
        for j in range(n_points):
            ...
            if div_id[i] < j < end_id[i] and merge_bool:
                new = (
                    np.asarray(other.GetPoint(j)) + np.asarray(line.GetPoint(j))
                ) / 2.0
...
def read_command_line_bifurcation(input_path=None, output_path=None):
    ...
    """
    description = (
        "Removes the bifurcation (possibly with an aneurysm), after which the"
    ...
    parser.add_argument(
        "--bif", type=str2bool, default=False, help="interpolate bif as well"
    )
    parser.add_argument(
        "--lower",
        type=str2bool,
    ...
    if not (-1 <= args.tension <= 1):
        raise ValueError(
            "Tension has to be between -1 and 1, not {}".format(args.tension)
        )

    if not (-1 <= args.continuity <= 1):
        raise ValueError(
            "Tension has to be between -1 and 1, not {}".format(args.continuity)
        )
    ...
    return dict(
        ...
        angle=ang_,
        keep_fixed_1=args.keep_fixed_1,
        keep_fixed_2=args.keep_fixed_2,
        bif=args.bif,
        lower=args.lower,
        cylinder_factor=args.cylinder_factor,
