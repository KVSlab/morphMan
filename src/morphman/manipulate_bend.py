...
def manipulate_bend(
    ...
    new_centerlines_path_tmp = (
        base_path + "_new_centerlines_alpha_%s_beta_%s_tmp.vtp" % (alpha, beta)
    )
    ...
    if diverging_centerline_ispresent:
        ...
        patch_diverging_line = get_clipped_diverging_centerline(
            extract_single_line(diverging_centerlines, 0),
            region_points[0],
            diverging_id,
        )
    ...
    centerline_remaining = create_parent_artery_patches(
        centerlines, region_points_vtk, siphon=True
    )
    ...
    if diverging_centerline_ispresent:
        ...
        centerline_bend = vtk_merge_polydata(
            [centerline_bend, diverging_centerline_end]
        )
    ...
    voronoi_bend, voronoi_remaining = get_split_voronoi_diagram(
        voronoi, [centerline_bend, centerline_remaining]
    )
    ...
    middle_points, _ = get_direction_parameters(
        centerlines, beta, direction, region_points_vtk
    )
    ...
    if beta != 0.0:
        ...
        voronoi_bend = move_voronoi_horizontally(
            dx_p1,
            voronoi_bend,
            centerline_bend,
            id1,
        ...
        if diverging_centerline_ispresent:
            new_centerlines = vtk_merge_polydata(
                [centerlines, extract_single_line(diverging_centerlines, 0)]
            )
        else:
    ...
    if alpha == 0.0 and beta != 0.0:
        ...
        region_points[0] = new_centerlines.GetPoint(
            locator.FindClosestPoint(region_points[0])
        )
        region_points[1] = new_centerlines.GetPoint(
            locator.FindClosestPoint(region_points[1])
        )

        # Vertical movement
        print("-- Moving geometry vertically")
        new_surface, new_centerlines = manipulate_bend_vertically(
            alpha,
            voronoi_remaining,
            voronoi_bend,
            new_centerlines,
    ...
    new_surface = prepare_output_surface(
        ...
        test_merge=True,
        changed=True,
        old_centerline=vtk_merge_polydata([centerlines, diverging_centerlines]),
    )
...
def manipulate_bend_vertically(
    alpha, voronoi_remaining, voronoi_bend, centerlines, region_points, poly_ball_size
):
    """
    ...
    middle_points, _, dx = get_direction_parameters(
        extract_single_line(centerlines, 0), alpha, direction, region_points_vtk
    )

    # Iterate over points P from Voronoi diagram and manipulate
    print("-- Adjust Voronoi diagram")
    voronoi_bend = move_voronoi_vertically(
        voronoi_bend,
        centerline_bend,
        id1,
        diverging_id,
        dx,
        diverging_centerline_ispresent,
    )
    new_voronoi = vtk_merge_polydata([voronoi_remaining, voronoi_bend])

    # Move centerline manually for postprocessing
    new_centerlines = get_manipulated_centerlines(
        centerlines, dx, p1, p2, diverging_id, diverging_centerlines, direction
    )
...
def move_voronoi_horizontally(
    ...
    clip_id,
    clip=False,
    diverging_centerline_ispresent=False,
):
    """
                ...
                if cl_id < idmid:
                    ...
                    if cl_id <= (id2 - 1):
                        ...
                        if clip_id < idmid_0:
                            ...
                            cl_id = clip_id - id1_0
                            dist = (
                                -dx_p1 * (cl_id - idmid) ** 0.5 / (id2 - idmid) ** 0.5
                            )
    ...
    new_dataset.GetPointData().AddArray(
        voronoi_clipped.GetPointData().GetArray(radiusArrayName)
    )

    return new_dataset


def move_voronoi_vertically(
    voronoi_clipped,
    centerline_clipped,
    id1_0,
    clip_id,
