...
from morphman.common.tools_common import convert_numpy_data_to_polydata
...
def get_maximum_coronal_coordinate(coronal_coordinate, curvilinear_coordinates):
    ...
    """
    value_index = coronal_coordinate[
        argrelextrema(coronal_coordinate, np.less_equal)[0]
    ].min()
    ...
    if (
        abs(
            curvilinear_coordinates[max_coronal_coordinate_id]
            - curvilinear_coordinates[-1]
        )
        > search_tolerance
    ):
        ...
        value_index = coronal_coordinate[
            argrelextrema(coronal_coordinate, np.greater_equal)[0]
        ].max()
        max_coronal_coordinate_id = np.array(
            coronal_coordinate.tolist().index(value_index)
        )
        if (
            abs(
                curvilinear_coordinates[max_coronal_coordinate_id]
...
def create_particles(base_path, algorithm, method):
    ...
    filename_all_landmarks = base_path + "_landmark_%s_%s.particles" % (
        algorithm,
        method,
    )
