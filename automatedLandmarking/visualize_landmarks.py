...
def combine_landmark( p):
    savepath = "/home/henrik/master_uio/results/landmark_results/%s" % p
    savedir = "./landmark_bog_%s.png" % p#"#"/home/henrik/master_uio/results/landmark_results/landmark_%s.png" % p
...
def viz_landmark(case, optimal, p):
    home = "/home/henrik/master_uio"
    pointpath = "%s/cases/%s/%s" % (home, case, optimal)
    siphonpath = "%s/cases/%s/surface/carotid_siphon.vtp" % (home, case)
    modelpath = "%s/cases/%s/surface/model.vtp" % (home, case)
    savepath = "../results/landmark_results/%s/%s.png" % (p, case)

    # Adjust points to lie on centerline
    try:
        tmp_particles = "tmp.particles"
        print siphonpath
        siphon = ReadPolyData(siphonpath)
    ...
    # reset view to fit data
    camerapath = "../results/camera/%s.pvcc" % case[2:]
...
def main():
    ...
    points = "BOGGY.particles"

    single = False
    p = "bogunovic"
    for case in cases:
        viz_landmark(case, points, p)
