...
def test_get_first_line_to_change(surface_paths):
    ...
    full_line_to_change = extract_single_line(
        read_polydata(base_path + "_centerline.vtp"), 0
    )
    line_to_change, _, _, _, _ = get_line_to_change(
        None, centerlines, "first_line", "", [], 0.0
    )
...
def test_get_end_point(surface_paths):
    ...
    last_point_true = np.asarray(
        centerline.GetPoint(centerline.GetNumberOfPoints() - 1)
    )
...
def test_reverse_centerline(surface_paths):
    ...
    point_end_r = reversed_centerline.GetPoint(
        reversed_centerline.GetNumberOfPoints() - 1
    )
