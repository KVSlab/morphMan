...
from morphman.common.tools_common import get_distance, get_path_names
...
def test_bifurcation_angle(surface_paths, angle):
    ...
    end_point1_old = np.array(
        extract_single_line(old_centerlines, cl_old_1).GetPoint(cl_old_id1 + 20)
    )
    end_point2_old = np.array(
        extract_single_line(old_centerlines, cl_old_2).GetPoint(cl_old_id2 + 20)
    )
    end_point1_new = np.array(
        extract_single_line(new_centerlines, cl_new_1).GetPoint(cl_new_id1 + 20)
    )
    end_point2_new = np.array(
        extract_single_line(new_centerlines, cl_new_2).GetPoint(cl_new_id2 + 20)
    )
    ...
    assert (
        abs(
            first_daughter_branch_angle_change
            + second_daughter_branch_angle_change
            - 2 * abs(common_input["angle"])
        )
        < 0.04
    )
