...
def test_compute_geometric_features(surface_paths):
    ...
    centerline = vmtk_compute_geometric_features(
        centerline, False, output_smoothed=False, factor=1.0, iterations=100
    )
...
def test_resample_centerline(surface_paths):
    ...
    assert (
        centerline_resampled_more.GetNumberOfPoints()
        < centerline_resampled.GetNumberOfPoints()
    )
