...
from morphman.common.centerline_operations import get_end_point
from morphman.common.tools_common import get_path_names
from morphman.common.vtk_wrapper import extract_single_line, read_polydata
...
def test_manipulate_branch_translation(surface_paths):
    ...
    common_input.update(
        dict(
            ...
            branch_to_manipulate_number=branch_number,
    ...
    old_centerlines = [
        extract_single_line(old_centerlines, i)
        for i in range(old_centerlines.GetNumberOfLines())
    ]
    new_centerlines = [
        extract_single_line(new_centerlines, i)
        for i in range(new_centerlines.GetNumberOfLines())
    ]
...
def test_manipulate_branch_translation_and_polar_rotation(surface_paths):
    ...
    new_centerlines0 = [
        extract_single_line(new_centerlines0, i)
        for i in range(new_centerlines0.GetNumberOfLines())
    ]
    ...
    new_centerlines1 = [
        extract_single_line(new_centerlines1, i)
        for i in range(new_centerlines1.GetNumberOfLines())
    ]
